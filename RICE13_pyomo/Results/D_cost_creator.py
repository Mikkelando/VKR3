...
def plot_comparison(region_code: str,
    ...
    coop_series_D = coop_data[region_code].loc['D']
    coop_series_Q = coop_data[region_code].loc['Q']

    non_coop_series_D = non_coop_data[region_code].loc['D']
    ...
    new_coop_data = [( coop_series_D.iloc[i]/ (1 + (coop_series_D.iloc[i])**10) ) * coop_series_Q.iloc[i] for i in range(len(coop_series_D))]
    new_noncoop_data = [( non_coop_series_D.iloc[i]/ (1 + (non_coop_series_D.iloc[i])**10) ) * non_coop_series_Q.iloc[i] for i in range(len(non_coop_series_D))]
    ...
    # Название оси Y
    y_label = unit_mapping.get('Damage Cost (Trillion $)', 'Damage Cost (Trillion $)')
    ...
    plt.title(f"Damage Cost — {region_code}")
    plt.xlabel("Year")
    plt.ylabel(y_label)
    plt.legend()
    plt.grid(True)
    ...
    # Сохранение
    filepath = os.path.join(folder, f"Damage_cost.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="D_cost", help="Папка для сохранения")
